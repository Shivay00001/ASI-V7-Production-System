...
- **Status**: {'✅ Excellent' if metrics['auroc'] > 0.85 else '⚠️ Good' if metrics['auroc'] > 0.75 else '❌ Needs Improvement'}
"""
        return summary

class BioMedicalFineTuner:
    """
    🔬 Advanced Bio-Medical Fine-Tuning Engine (V5.0 Integration)
    """
    def __init__(self, system_instance):
        self.system = system_instance
        
    def incremental_train(self, new_patient_df, target_column='treatment_response'):
        """
        Update the predictive models with new patient data.
        """
        print(f"🔄 [V5.0] Processing {len(new_patient_df)} new clinical samples...")
        
        # Preprocess
        # Note: Accessing internal preprocess method requires care in production
        # Here we assume compatibility
        X_new = new_patient_df.drop(columns=[target_column])
        y_new = new_patient_df[target_column]
        
        if self.system.model:
             print("💉 Refining decision boundaries with new genotype-phenotype correlations...")
             if hasattr(self.system.model, 'fit'):
                 # Attempt warm start if supported, otherwise re-fit
                 try:
                     self.system.model.fit(X_new, y_new, xgb_model=self.system.model)
                 except:
                     self.system.model.fit(X_new, y_new)
        else:
            print("⚠️ Initializing new model training context...")
            self.system._train_model(X_new, y_new, 'xgboost')
            
        print("✅ Clinical Knowledge Graph Updated.")

def attach_tuner(system):
    system.tuner = BioMedicalFineTuner(system)
    return system